...
phase_sum = zeros(16,500);

for window = 1:length(new_group_b{1,1,1})/win_size
    for i = 1:20
        if length(new_group_b{i,1,1}) == 91000
            in_data(k,:) = bandpass(new_group_b{i,1,1}(1+(window-1)*win_size:500+(window-1)*win_size),[8 13],fs);
            k = k+1;
        end
    end
    k = 1;
    ...
    phase_sum = phase_sum + phase_data;
    
end

phase_mean = phase_sum/182;

figure('Name','Phase Lag Index');

for plotId = 1:height(pairs)
    subplot(3,5,plotId);
    polarAngleDiffH = polarplot(repmat(phase_mean(pairs(plotId,1),:)-phase_mean(pairs(plotId,2),:)*1,2)',repmat([0 1],1,length(phase_data))','k');
